  ...
  <div class="app-container">
    ...
    <div class="div-table">
      <div class="search-form" style="margin-bottom:15px;">
        <el-input v-model="searchForm.name" size="small" clearable placeholder="用户名称" style="width:200px;" />
        <el-input v-model="searchForm.realName" size="small" clearable placeholder="真实姓名" style="width:200px;" />
        <el-select v-model="searchForm.userType" size="small" clearable filterable placeholder="请选择用户类型">
          <el-option
      ...
      >
        ...
        <el-table-column class-name="status-col" label="用户类型" width="80">
          <template slot-scope="scope">
            <el-tag>{{ scope.row.userType | statusFilter }}</el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="activeTitle" :visible.sync="dialogFormVisible">
      <el-form ref="ruleForm" :model="form" :rules="formrules">
        <el-form-item label="所属部门" :label-width="formLabelWidth" prop="departId">
          <el-select v-model="form.departId" size="small" clearable filterable placeholder="请选择所属部门">
            <el-option
        ...
        <el-form-item label="用户类型" :label-width="formLabelWidth" prop="userType">
          <el-select v-model="form.userType" size="small" clearable filterable placeholder="请选择用户类型">
            <el-option
              v-for="item in userTypeMaps"
              :key="item.id"
              :label="item.name"
      ...
      <el-form :model="form">
        <el-form-item label="角色授权" :label-width="formLabelWidth">
          <el-checkbox-group v-model="authRole">
            <el-checkbox v-for="(item, index) in roleList" :key="index" :label="item.id" :disabled="item.status === 2">{{ item.name }}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
...
import { getList, getDetail, getRoleInfo, saveRoleInfo, save, del } from '@/api/systemapi/user'
  ...
  methods: {
    ...
    setAuth(data) {
      ...
      getRoleInfo({ userId: data.id }).then(response => {
        ...
        this.authRole = this.authForm.roleids ? this.authForm.roleids.split(',').map(Number) : []
      })
    },
    saveRole() {
